...
def run_benchmark(subfolder_name, slot_count, w_ops, w_keys, build_path, poly_args=None):
    ...
    if poly_args:
        cmd = f"./{subfolder_name} {poly_args} 1 1 0 1 1 1 {w_ops} {w_keys}"
    else:
        cmd = f"./{subfolder_name} 1 {slot_count} 1 0 1 1 1 {w_ops} {w_keys}"
        
    try:
        res = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, cwd=build_path, timeout=7200)
        
        # --- SECURITE: Afficher l'erreur si l'agent Python ou le C++ plante ---
        if res.returncode != 0:
            print(f"\n[ERREUR CRITIQUE] Le benchmark {subfolder_name} a planté !")
            print(f"--- Raison du crash (Stderr) ---\n{res.stderr.strip()}\n--------------------------------")
        
        for line in res.stdout.splitlines():
            clean = re.sub(r'\x1b\[[0-9;]*m', '', line)
            
            # --- FIX: Extraction robuste pour les deux dernières métriques ---
            ops_match = re.search(r'(?:final\s*ops\s*cost|Final\s*exec\s*cost)\s*:\s*([\d\.]+)', clean, re.IGNORECASE)              
            if ops_match: stats["final_ops_cost"].append(float(ops_match.group(1)))
            
            keys_match = re.search(r'final\s*keys\s*cost\s*:\s*([\d\.]+)', clean, re.IGNORECASE)
            if keys_match: stats["final_keys_cost"].append(float(keys_match.group(1)))
        ...
        if res.returncode == 0:
            ...
            run_res = subprocess.run("./main", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, cwd=build_he_path)
        ...
        if os.path.exists(cpp_file):
            with open(cpp_file, "r") as f:
                content = f.read()
                for op in operations: stats[op].append(len(re.findall(rf'\b{op}', content)))
    except Exception as e:
        print(f"Exception Python critique lors de l'exécution de {subfolder_name}: {e}")
...
for sub in benchmark_folders:
    ...
    if os.path.isdir(b_path):
        for sc in benchmarks_slot_counts.get(sub, slot_counts):
            print(f"*****run {sub} , slot : {sc}******")
            run_benchmark(sub, sc, 0.0, 1.0, b_path)
    ...
    for reg in regimes:
        for d in depths:
            for inst in range(1, number_instances_each_polynomial_configuration + 1):
                print(f"*****run tree_{reg}_{d}_{inst}******")
                run_benchmark("polynomials_coyote", 1, 0.0, 1.0, poly_path, poly_args=f"{d} {inst} {reg}")